...
def index():
    ...
    pagination = query.order_by(Post.timestamp.desc()).paginate(
        page,per_page = current_app.config['TEST_POSTS_PER_PAGE'],error_out=False)
...
def post(id):
    ...
    if page == -1:
        page = (post.comments.count() - 1)//\
               current_app.config['TEST_COMMENTS_PER_PAGE'] + 1
    pagination = post.comments.order_by(Comment.timestamp.asc()).paginate(
        page,per_page=current_app.config['TEST_COMMENTS_PER_PAGE'],error_out=False
    )
...
def unfollow(username):
    ...
    db.session.commit()
    flash('取消关注成功，您可以重新关注%s'%username)
    return  redirect(url_for('.user',username=username))
...
def followers(username):
    ...
    if user is None:
        ...
        return redirect(url_for('.index'))
    page = request.args.get('page',1,type=int)
    pagination = user.follower.paginate(page,per_page=current_app.config['TEST_FOLLOWERS_PER_PAGE'],error_out=False)
...
def followed_by(username):
    ...
    if user is None:
        ...
        return redirect(url_for('.index'))
    page = request.args.get('page',1,type=int)
    pagination = user.followed.pagination(page,per_page=current_app.config['TEST_FOLLOWERS_PER_PAGE'],error_out = False)
    follows = [{'user':item.followed,'timestamp':item.timestamp} for item in pagination.items]
    return render_template('followers.html',user=user,title='关注',endpoint = '.followed',pagination=pagination,follows=follows)
